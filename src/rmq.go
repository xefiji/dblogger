...
func (r *Rmq) Receive(from string) <-chan amqp.Delivery {
	...
	var q amqp.Queue
	switch true {
	case exchangeIsInternal(from): //internal: get all messages from direct exchange
		r.exchangeDeclare(from, DIRECT)
		q := r.queueDeclare(from, true, false, false)
		r.queueBind(q.Name, INTERNAL_EXCHANGE, from)
	case exchangeIsPublic(from): //default (public): subscribe to all messages from fanout exchange
